...
const CategoryPage = ({ searchResults, searchQuery }) => {
	...
	useEffect(() => {
		const fetchArticles = async () => {
			try {
				...
				let fetchedArticles = [];
				if (searchQuery) {
					fetchedArticles = searchResults.filter(
						(article) =>
							article.category.toLowerCase() === category.toLowerCase()
					);
				} else if (category) {
					fetchedArticles = await fetchArticlesByCategory(category);
				} else {
					fetchedArticles = await fetchAllArticles();
				}
				console.log("Fetched Articles in CategoryPage:", fetchedArticles);
		...
		fetchArticles();
	}, [category, searchResults, searchQuery]);
